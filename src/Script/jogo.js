...
function createConfirmModal(message) {
    return new Promise((resolve) => {
        // Crie o overlay principal
        const overlay = document.createElement('div');
        overlay.classList.add('modal-overlay');

        // Crie o modal de confirmação, que ficará DENTRO do overlay
        const modal = document.createElement('div');
        ...
        // Anexe o modal de confirmação DENTRO do overlay
        overlay.appendChild(modal);

        // Anexe o overlay completo ao corpo da página
...
function createAddGameModal(isEditing = false, gameData = {}) {
        ...
        <div class="modal-content" id="first-page">
            ...
            <form id="add-game-form">
                <div class="form-group">
                    ...
                    <input type="text" id="Nome_jogo" name="Nome_jogo" required value="${gameData.Nome_jogo || ''}">
                </div>
                <div class="form-group">
                    <label for="Descricao_jogo">Descrição:</label>
                    <textarea id="Descricao_jogo" name="Descricao_jogo" required>${gameData.Descricao_jogo || ''}</textarea>
                </div>
                <div class="form-group">
                    <label for="Preco_jogo">Preço (R$):</label>
                    <input type="number" id="Preco_jogo" name="Preco_jogo" step="0.01" required value="${gameData.Preco_jogo || ''}">
                </div>
                <div class="form-group">
                    <label for="Logo_jogo">Logo (URL da Imagem):</label>
                    <input type="url" id="Logo_jogo" name="Logo_jogo" value="${gameData.Logo_jogo || ''}">
                </div>
                <div class="form-group">
                    <label for="Capa_jogo">Capa (URL da Imagem):</label>
                    <input type="url" id="Capa_jogo" name="Capa_jogo" required value="${gameData.Capa_jogo || ''}">
                </div>
                <div class="form-group">
    ...
    const secondPageHTML = `
        <div class="modal-content" id="second-page" style="display: none;">
            ...
            <div class="form-buttons">
                ...
                <button type="button" class="submit-btn">${isEditing ? "Salvar Edições" : "Adicionar Jogo"}</button>
            </div>
        </div>
    `;
    ...
    const setupMediaPreview = () => {
        ...
        existingMidiasToKeep.forEach((mediaUrl) => {
            ...
            mediaElement.querySelector('.remove-media-btn').addEventListener('click', () => {
                mediaElement.remove();
    ...
    submitBtn.addEventListener('click', async (e) => {
        ...
        genreMultiselect.getValues().forEach(g => formData.append("generos[]", g));
        categoryMultiselect.getValues().forEach(c => formData.append("categorias[]", c));
        ...
        if (!jogoId) {
            ...
            showMessage("Não foi possível salvar as edições. ID do jogo não encontrado.");
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Erro ao salvar as edições do jogo.');
            }
            ...
            showMessage("Ocorreu um erro ao salvar as edições do jogo. " + error.message);
        }
    });
...
function setupSwiperSliders(game) {
    ...
    if (typeof Swiper !== "undefined") {
        var swiperThumbs = new Swiper(".jogo-side-image", {
            spaceBetween: 16,
            slidesPerView: 4,
            freeMode: true,
            loop: false,
        ...
        var swiperMain = new Swiper(".jogo-main-image", {
            spaceBetween: 10,
            autoplay: {
                delay: 2500,
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                document.querySelector("main").innerHTML = `<p style="color: red; text-align: center;">Jogo não encontrado. Verifique se o ID ${jogoId} existe no banco de dados. Certifique-se de que o back-end está rodando.</p>`;
                return;
            }
            const errorData = await response.json();
